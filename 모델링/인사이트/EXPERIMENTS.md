...
**배포**: `submit/v12_rmo_full_meta/submit.zip` 빌드, sanity(빌드시점 5행 재현값과 격리 환경 재실행 결과 일치) 확인 완료. E029가 로컬 유의성 기준 미달에도 실제로 크게 개선됐던 전례가 있어 이 확장판도 실제 제출로 검증 예정 — 결과 나오면 이 항목 업데이트.
