...
**후속 조정 시도** (`segment_corrector_rmo_logratio_tuning.py`) — mr/or 단독 사용, corrector capacity(min_samples_leaf) 강화 4가지 조합을 원본과 나란히 재검증:

| 피처 조합 / capacity | PRIMARY (z) | STRESS (z) |
|---|---:|---:|
| both / 기본(min_leaf=200) — **E029 원본** | 811.45 (1.71) | 768.56 (2.01) |
| both / 정규화강화(min_leaf=400) | 808.51 (1.17) | 761.96 (0.91) |
| mr만 / 기본 | 800.51 (-0.30) | 764.99 (1.56) |
| mr만 / 정규화강화 | 804.67 (0.58) | 757.43 (0.28) |
...
## E030 (2026-08-18) — E029 메타피처 확장: log-ratio 2개 → 원시확률+log-ratio 6개 (`개발/v3_domain_experiments/segment_corrector_rmo_extended_meta.py`) — PRIMARY 노이즈 수준, STRESS 강하게 유의, 제출 대기

E029(champion, 911.19)가 실제로 통한 걸 확인한 뒤, "비율(모양)"만 주던 메타피처를 "크기(확신도)"까지 포함하도록 확장. hazard 서브모델(qR/qM/qO) 자체는 E029와 동일(재사용), 반환값만 확장: 원시확률 3개(qR/qM/qO) + log-ratio 3개(mr=log(qM/qR), or=log(qO/qR), **om=log(qO/qM) 신규**). 비교 기준은 baseline이 아니라 **champion(E029, mr+or 2개)**.

| 메타피처 조합 | PRIMARY (z vs champion) | STRESS (z vs champion) |
|---|---:|---:|
| champion(mr+or) | 811.45 (기준) | 768.56 (기준) |
| +raw(qR,qM,qO) | 810.72 (z=-0.19) | 802.07 (**z=4.52**, +33.51) |
| +3rd log-ratio(om) | 806.53 (z=-1.39) | 774.41 (z=1.22) |
| **전부(6개)** | 809.49 (z=-0.44) | **810.39 (z=5.35, +41.83)** |

"전부(6개)" 조합이 최선: PRIMARY는 champion과 통계적으로 구별 안 되는 노이즈 수준(z=-0.44)이고, STRESS는 z=5.35로 강하게 유의하게 개선됐다. E029(PRIMARY z=1.71로 유의 기준 미달, 즉 "약한 개선")보다 오히려 더 안전한 프로필이다(PRIMARY가 나빠진 게 아니라 그냥 그대로, STRESS만 크게 좋아짐).
...
**배포**: `submit/v13_joint_softmax_meta/submit.zip` 빌드, sanity(빌드시점 5행 재현값과 격리 환경 재실행 결과 일치) 확인 완료.
...
## E035 (2026-08-20) — Trackman 물리데이터를 hazard 서브모델 입력으로 (`개발/v3_domain_experiments/segment_corrector_trackman_hazard_input.py`) — 약한 양의 신호, 유의성 미달로 보류/기각

E024에서 Trackman 12개 파생변수를 champion residual과 직접 상관관계로 검증해 완전 기각됐던 것과 다른 카테고리 시도 — "물리 데이터가 성공/실패 자체는 못 바꿔도 실패 유형(어느 쪽으로 벗어나는가)은 바꿀 수 있다"는 가설로, `trackman_features.py`(우리 자체 매핑, 신뢰 332명/커버리지 68.4%)의 12개 피처를 hazard 서브모델(CatBoost) 입력에 추가해서 재학습.

| | champion(v14, 메타피처 13개) | +Trackman hazard | 차이 | z |
|---|---:|---:|---:|---:|
| PRIMARY | 806.37 | 808.19 | +1.83 | 0.40 |
| STRESS | 846.30 | 852.50 | +6.20 | 1.09 |

**약한 양의 신호는 있지만(두 폴드 다 방향은 개선) 유의성은 E029(z=1.71)보다도 약해서 채택 기준에 못 미친다.** 가설 자체(물리 데이터 → 실패 유형)의 방향은 틀리지 않았을 가능성이 있지만(완전한 노이즈라면 음의 방향도 나왔을 것), 신호 크기가 너무 작아 실제 제출로 검증할 만큼의 확신은 없다. **보류(재시도 시 표본을 커버리지 68.4%로 제한한 subset 검증이나 신뢰도 임계값 조정으로 다시 볼 수 있음), 이번 라운드는 기각.**

## E036 (2026-08-20) — LSTM을 hazard 서브모델로 (`개발/v3_domain_experiments/segment_corrector_lstm_hazard_meta.py`) — 폴드 부호가 갈려서 기각

사용자 요청 "시계열적으로 잘 맞는 모델도 넣어보자" — E020(diversity_lab_lstm.py v2, 이미 검증된 윈도우/정규화 로직)을 그대로 재사용해서 타겟만 control_success에서 R/M/O 3단계로 교체. E020은 이 로직으로 control_success를 직접 예측했을 때 BSS=0.00(신호 없음)이었는데, R/M/O 서브 타겟은 다를 수 있는지 확인.

| | champion(v14, 메타피처 13개) | +LSTM hazard | 차이 | z |
|---|---:|---:|---:|---:|
| PRIMARY | 781.94 | 787.90 | +5.96 | **1.71** |
| STRESS | 848.35 | 844.84 | -3.51 | **-0.74** |

**폴드 부호가 갈려서 기각.** PRIMARY는 E029 수준의 양의 신호(z=1.71)인데 STRESS는 오히려 음의 방향(z=-0.74)이다 — 이 프로젝트가 처음부터 지켜온 "로컬 폴드 부호가 갈리면 못 믿는다" 원칙(E006에서 확립)에 정확히 걸리는 사례. E029/E030/E032/E033(champion이 된 것들)은 전부 두 폴드가 같은 방향이었고 유의성 강도만 달랐다는 점에서 이번과 결정적으로 다르다. LSTM 자체가 표본이 적은 서브 타겟(특히 STRESS의 outside_label 학습 표본 65만행)에서 과적합했을 가능성, 혹은 정말 신호가 없어서 폴드마다 다른 노이즈를 학습했을 가능성 둘 다 있음 — 추가 투자 안 하고 종료.

## E037 (2026-08-20) — Ridge hazard를 champion(Lasso 포함) 위에 추가로 (`개발/v3_domain_experiments/segment_corrector_ridge_addon.py`) — 명확히 기각

E033에서 Ridge 단독은 STRESS z=1.82로 Lasso(z=2.15) 다음으로 근접한 후보였다. Lasso가 이미 champion(v14)에 들어간 상태에서, Ridge를 **추가로** 얹으면 더 좋아지는지 확인.

| | champion(v14, Lasso 포함 메타피처 13개) | +Ridge hazard | 차이 | z |
|---|---:|---:|---:|---:|
| PRIMARY | 816.23 | 809.40 | -6.83 | **-1.82** |
...
## E040 (2026-08-21) — 팀원 P997 "diversity stack" 구조를 독립 재구현해 corrector 메타피처로 (`개발/v3_domain_experiments/segment_corrector_diversity_stack_meta.py`) — 개별 성분은 약하지만 5개 전부 합치니 3폴드 전부 유의, 이 세션 최고 승리 패턴

공식 병합된 팀원 pipeline의 P997("서로 다른 피처 부분집합 × 서로 다른 알고리즘으로 같은 잔차 타겟을 5번 따로 맞혀서 섞는" 구조)을 코드는 안 가져오고 구조만 참고해서 독립 재구현(대회 규정 §11). 원본은 D 성분이 LightGBM인데, E026("잘 튜닝한 LightGBM은 CatBoost와 residual corr 0.9996으로 거의 같은 실수")을 이미 확인했으므로 LightGBM 대신 **CatBoost(l2_leaf_reg=L2_LEAF_REG)**로 대체(사용자 지시). 나머지 4개(FAILURE/PITCHMIX/BASE_STATE=ExtraTrees, RFSTATE=RandomForest)는 팀원 구조 그대로, 피처 부분집합만 우리 44피처 이름으로 매핑. target은 팀원처럼 "도메인평균 대비 잔차"가 아니라 우리 corrector가 이미 쓰는 champion 잔차(base 예측 대비)를 그대로 사용 — 5개 성분을 corrector 메타피처 자리에 위치시킴(hazard/joint/lasso와 동급). 리크 방지를 위해 5개 성분은 train_df를 한 번 더 안쪽에서 쪼갠(inner-split, residual-source 패턴과 동일) 잔차로 학습.

| | champion(v14, 13개) | +dstack_d | +dstack_failure | +dstack_pitchmix | +dstack_rfstate | +dstack_basestate | **+전부(5개)** |
|---|---:|---:|---:|---:|---:|---:|---:|
| 2024est | 817.63 | 815.41(z=-0.62) | 824.25(z=1.20) | 816.36(z=-0.29) | 813.22(z=-1.32) | 809.23(z=-2.23) | **832.09(z=2.53)** |
| 2023est | 857.17 | 848.58(z=-1.82) | 862.74(z=1.09) | 863.35(z=0.88) | 848.67(z=-1.76) | 872.27(z=2.54) | **883.61(z=3.29)** |
| 2022est | 2454.62 | 2461.35(z=1.37) | 2462.91(z=1.24) | 2458.43(z=0.80) | 2450.76(z=-0.89) | 2457.92(z=0.76) | **2478.79(z=3.36)** |

**개별 성분은 폴드마다 방향이 들쭉날쭉하다**(dstack_basestate만 봐도 2024est는 z=-2.23인데 2023est는 z=2.54로 정반대). 그런데 **5개를 전부 corrector에 동시에 넣으면 3개 폴드 전부, 그것도 전부 유의 기준(z≈1.96)을 넘겨서 개선**된다 — E029(PRIMARY 미달)/E030/E032/E033(항상 한쪽 폴드만 확실)보다도 깨끗한, 이 세션 유일의 "3폴드 전부 유의" 사례. 개별 성분이 노이즈에 가까워도 서로 다른 피처 부분집합×알고리즘 조합이 모이면 corrector(ExtraTrees)가 알아서 유용한 부분만 골라 쓰는 것으로 해석된다 — 팀원 pipeline이 이 구조를 굳이 5개나 쓰는 이유와 같은 맥락.

**채택, v16으로 패키징 예정.**

## E041 (2026-08-22) — pressure_backoff/diversity stack의 학습 타겟을 base 잔차에서 corrector 잔차로("진짜 캐스케이드") (`개발/v3_domain_experiments/segment_corrector_true_cascade.py`) — 폴드마다 결과 갈림, 보류

사용자 요청: "base는 우리꺼 쓰고 2단 이후로 3단부터는 팀원의 논리대로" — 지금까지(E038/E040) pressure_backoff/diversity stack은 전부 **base 잔차**(residual1 = y - base_pred)를 목표로 학습해서 하나의 flat corrector에 메타피처로 욱여넣었다. 이번엔 이 둘의 학습 타겟을 **2단(hazard+joint+lasso corrector)이 설명하고 남긴 잔차**(residual2 = y - pred2)로 바꿔서, "이전 단의 잔차를 다음 단이 교정한다"는 팀원 pipeline의 캐스케이드 철학을 반영. leak-safety를 위해 table_target(train_df의 마지막 연도)에서 base+corrector 전체를 한 번 더(1년 더 이전 데이터로) 재현해서 그 잔차를 구함(재귀는 1단계로 한정). 최종 결합은 기존처럼 하나의 flat corrector(residual1 목표, cross-fit)로 유지 — 바뀐 건 pressure_backoff/dstack 서브모델이 "무엇을 맞히도록 학습됐는가"뿐.

| | champion(2단) | 기존(서브모델이 residual1 목표, v15/v16) | 신규(서브모델이 residual2 목표, 캐스케이드) |
|---|---:|---:|---:|
| 2024est | 818.49 | 828.22 (z=1.48) | **851.88 (z=4.40)** |
| 2023est | 857.86 | **893.28 (z=4.03)** | 892.03 (z=3.56) |
| 2022est | 2454.42 | **2478.42 (z=2.76)** | 2466.06 (z=1.17) |

**폴드마다 결과가 갈렸다.** 2024est에서는 캐스케이드가 기존보다 뚜렷하게 더 좋았지만(z 1.48→4.40), 2023est는 거의 동률(기존이 근소 우위), 2022est는 기존이 뚜렷하게 더 좋았다(캐스케이드가 오히려 손해). E040(3폴드 전부 유의)만큼 깨끗하지 않다. 추정 원인: 캐스케이드의 table_target 잔차 재현 과정 자체가 "table_source(그보다 1년 더 이전 데이터)로 학습한 base+corrector"에 의존하는데, 2022est 폴드는 table_source가 2019~2020(2년치)뿐이라 이 재귀 단계 자체가 얕은 데이터 위에서 이뤄져 노이즈가 컸을 가능성 — 2024est/2023est의 table_source(3~4년치)보다 훨씬 불안정. **결론(잠정)**: "이전 단 잔차를 목표로 재학습"하는 캐스케이드 방향 자체는 유망하지만(2024est 결과가 이 세션 최고 z값인 4.40), 재귀에 쓰이는 table_source 데이터가 충분히 두꺼울 때만 안정적으로 통하는 것으로 보임 — 데이터가 얕은 폴드에서는 오히려 해로울 수 있어 **보류(기각 아님)**. 사용자 지시로 이후 작업 일시 중단, 재개 시 여기부터 계속.
