...
## E021 (2026-08-14) — hand match를 corrector segment 라우팅 축으로 추가 — 로컬 두 폴드 다 개선, 그러나 신뢰도 낮음
...
| | base | 3-way(기존 champion) | 6-way(+hand routing) | 차이 |
|---|---:|---:|---:|---:|
| PRIMARY | 734.49 | 801.93 | **821.64** | **+19.71** |
| STRESS | 10.25 | 755.63 | **781.96** | **+26.34** |
...
**로컬 두 폴드 다 확실히 개선(원칙상 채택 기준 통과).** 다만 **강한 경고**: 이 신호는 E023(residual bias 스캔)이 이미 찾아서 "hand_matchup과 동일 신호, 로컬은 좋았는데 실제 LB에서 하락한 전례가 있다"고 명시적으로 경고했던 바로 그 신호다. 이번엔 인코딩 방식(피처가 아니라 segment 라우팅)을 바꿨을 뿐, 활용하는 **근본 정보(투수손x타자손)는 동일**하다 — 인코딩을 바꿨다고 그 정보 자체의 2025 일반화 안정성이 달라진다는 보장은 없다.

**결론**: 로컬 채택 기준(두 폴드 모두 개선)은 통과하지만, 이미 한 번 실LB에서 뒤집힌 것과 같은 신호원이라 신뢰도를 다른 신규 발견과 동일하게 볼 수 없다. **실제 제출 후보로 올리려면 사용자 판단이 필요 — 자동으로 champion 교체하지 않음.** 코드는 재현 가능하게 남겨둔다.
