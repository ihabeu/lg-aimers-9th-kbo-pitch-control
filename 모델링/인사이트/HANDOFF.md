...
**⚠️ 2026-08-20부터 `submit/`(실제 제출 코드/학습된 모델)은 비공개 저장소로 이관됨**: `github.com/ihabeu/lg-aimers-9th-kbo-private` (비공개, 구 이름 `lg-aimers-9th-kbo-submissions`). 이 문서와 `EXPERIMENTS.md`에 나오는 `submit/vN_.../submit.zip` 경로는 이제 이 공개 저장소가 아니라 그 비공개 저장소를 가리킨다 — 실제 제출 코드가 활성화된 대회 중 그대로 공개되는 걸 막기 위함.

**⚠️ 2026-08-20 공식 팀 병합으로 팀원 pipeline 코드를 공유받은 시점(E038)부터는 방법론/실험 로그도 비공개 저장소로 이관됨**(`lg-aimers-9th-kbo-private`의 `모델링/인사이트/`). 15차 제출 이후의 실험/제출 이력은 이 문서가 아니라 그쪽을 참고. E001~E037(champion v14, 938.97까지)은 계속 이 공개 저장소에 남아있다.
...
| 15차 이후 | 공식 팀 병합으로 팀원 pipeline 공유받은 시점부터 진행 | — | private 저장소(`lg-aimers-9th-kbo-private`) 참고 |
