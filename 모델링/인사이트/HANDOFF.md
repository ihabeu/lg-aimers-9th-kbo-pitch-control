...
| 14차 (2026-08-19) | 13차와 동일 구조 + Lasso(L1 로지스틱) hazard 서브모델 log-ratio 3개 추가(메타피처 총 13개) — "모델을 바꾸든 섞든" 요청으로 LightGBM/Ridge/Lasso를 hazard 서브모델로도 만들어 비교(E033), 메인 타겟에서는 CatBoost와 거의 동일했던 LightGBM이 R/M/O 서브 타겟에서는 상관 0.65~0.96, Ridge/Lasso는 0.60~0.89로 더 이질적임을 확인 | champion/hazard/joint softmax/Lasso 서브모델 전부 2019~2024 전체 재학습 — `submit/v14_lasso_hazard_meta/submit.zip` | **938.9722770746 (2026-08-21 시점 실제 확인된 최선, 유효 champion)** — 919.80 대비 **+19.17**. 로컬(E033) PRIMARY 노이즈(z=-0.12)/STRESS 유의(z=2.15, +10.89)로 E029/E030/E032와 같은 패턴이었는데, 실제 개선폭은 E030(+4.01)/E032(+4.60)보다 훨씬 컸다 — **"메타피처를 더 늘리는" 것보다 "메타피처 소스 자체(다른 알고리즘)를 다양화하는" 게 더 큰 정보였다는 뜻.** 수확체감이 단조롭지 않다는 반례. |
| 15차 (2026-08-21, 제출 대기) | 14차와 동일 구조 + pressure_backoff(공식 병합된 팀원 pipeline N322 참고, 3단 계층 crossed-EB backoff) 메타피처 1개 추가(총 14개) — 코드는 안 가져오고 공식/k값만 참고해 독립 재구현 | champion/hazard/joint softmax/Lasso/pressure_backoff 전부 2019~2024 전체 재학습 — `submit/v15_pressure_backoff/submit.zip` | **아직 실제 제출 안 함.** 로컬(E038, 폴드명 신규: 2024검증/2023검증) 2024검증 815.66→827.40(z=2.41, 유의), 2023검증 857.99→867.99(z=1.73, 근접/동일방향) — E029와 같은 승리 패턴. |
| 16차 (2026-08-21, 제출 대기) | 15차와 동일 구조 + diversity stack(공식 병합된 팀원 pipeline P997 구조 참고, 서로 다른 피처 부분집합×알고리즘 5개로 같은 잔차 타겟을 따로 맞혀서 섞음) 메타피처 5개 추가(총 19개) — 원본 D 성분(LightGBM)은 E026 근거로 CatBoost(l2_leaf_reg)로 대체, 나머지 4개(FAILURE/PITCHMIX/BASE_STATE=ExtraTrees, RFSTATE=RandomForest)는 구조 그대로 재구현 | champion/hazard/joint softmax/Lasso/pressure_backoff/diversity stack 전부 2019~2024 전체 재학습 — `submit/v16_diversity_stack/submit.zip` | **아직 실제 제출 안 함.** 로컬(E040, 3폴드로 확장: 2024est/2023est/2022est) 개별 성분은 폴드마다 방향이 갈리지만 5개 전부 넣으면 **3개 폴드 전부 유의**(z=2.53/3.29/3.36) — 이 세션 유일의 3폴드 전부 유의 사례. |
