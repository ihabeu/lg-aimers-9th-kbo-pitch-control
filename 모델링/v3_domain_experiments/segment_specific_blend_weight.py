"""
multimodel_weighted_blend.py에서 전체에 단일 가중치(0.6/0.2/0.2)를 썼는데, 이미 segment(core/
hybrid/dev)마다 base/corrector 성격이 크게 다르다는 걸 알고 있다(README의 segment별 개선폭 참고).
segment별로 base 블렌드 가중치 자체도 다르게 주면 더 나을 수 있는지 확인한다.

먼저 segment 내부에서 어느 가중치가 raw blend brier를 가장 낮추는지(corrector 없이, 싸게) 찾고,
그 조합을 assemble해서 기존 global-best(0.6/0.2/0.2)와 corrector까지 포함한 최종 점수로 비교한다.
"""
import sys
...
def run_fold(df, target_season, label):
    ...
    # segment별로 raw blend brier(corrector 없이)가 최소인 가중치 찾기 -- 값싸다, 재학습 불필요
    best_per_segment = {}
    print("  segment별 raw blend 최적 가중치 (corrector 전):")
    for seg in SEGMENTS:
        mask = segment == seg
        y_seg = y[mask]
        best_w, best_brier = None, np.inf
        for w in WEIGHT_GRID:
            p_blend = np.clip(w[0] * p_cat[mask] + w[1] * p_lgb[mask] + w[2] * p_xgb[mask], 0, 1)
            brier = float(np.mean((p_blend - y_seg) ** 2))
            if brier < best_brier:
                best_brier, best_w = brier, w
        best_per_segment[seg] = best_w
        print(f"    {seg}: best={best_w} brier={best_brier:.6f}")

    # segment별 최적 가중치를 조합한 블렌드 예측 생성
    p_blend_segwise = np.zeros(len(valid_df))
    for seg in SEGMENTS:
        mask = segment == seg
        w = best_per_segment[seg]
        p_blend_segwise[mask] = w[0] * p_cat[mask] + w[1] * p_lgb[mask] + w[2] * p_xgb[mask]
    p_blend_segwise = np.clip(p_blend_segwise, 0, 1)

    p_blend_global = np.clip(GLOBAL_BEST[0] * p_cat + GLOBAL_BEST[1] * p_lgb + GLOBAL_BEST[2] * p_xgb, 0, 1)

    X_corr = corrector_matrix(valid_df)
    s_global = crossfit_score(X_corr, y - p_blend_global, segment, p_blend_global, y, valid_df)
    s_segwise = crossfit_score(X_corr, y - p_blend_segwise, segment, p_blend_segwise, y, valid_df)
    print(f"  전체 단일 가중치(0.6/0.2/0.2)+corrector: {s_global:.2f}")
    print(f"  segment별 가중치+corrector:              {s_segwise:.2f}  (차이 {s_segwise - s_global:+.2f})")
    return s_global, s_segwise
...
def main():
    ...
    print("\n===== 두 폴드 동시 비교 =====")
    print(f"  PRIMARY: global={r1[0]:.2f}  segwise={r1[1]:.2f}")
    print(f"  STRESS:  global={r2[0]:.2f}  segwise={r2[1]:.2f}")
