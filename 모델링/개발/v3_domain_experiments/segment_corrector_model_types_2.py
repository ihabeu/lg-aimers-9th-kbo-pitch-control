"""
segment_corrector_model_types.py 이어서 -- RandomForest(ExtraTrees와 같은 배깅 계열, bootstrap
샘플링 + 분할점은 최선을 찾음)와 LightGBM(XGBoost와 같은 부스팅 계열, leaf-wise 성장이라 XGBoost와는
구현이 다름)을 corrector로 추가 비교.
"""
import sys
...
RF_CFG = dict(n_estimators=100, max_depth=10, min_samples_leaf=200, max_features=0.7)
LGB_CFG = dict(n_estimators=150, max_depth=5, learning_rate=0.05, subsample=0.8, colsample_bytree=0.8,
               reg_lambda=5.0, min_child_samples=200, num_leaves=15, verbosity=-1)
...
def run_fold(df, target_season, label):
    ...
    makers = {
        "ExtraTrees": lambda s: ExtraTreesRegressor(n_jobs=-1, random_state=16200 + int(s), **ET_CFG),
        "RandomForest": lambda s: RandomForestRegressor(n_jobs=-1, random_state=16200 + int(s), **RF_CFG),
        "LightGBM": lambda s: lgb.LGBMRegressor(random_state=16200 + int(s), n_jobs=-1, **LGB_CFG),
    }
    results = {}
    preds = {}
    for name, maker in makers.items():
        seed_preds = []
        for seed in SEEDS:
            corr = crossfit_correction(maker, X, residual, segment, valid_df, seed)
            seed_preds.append(np.clip(base_pred + corr, 0, 1))
        consensus = np.mean(np.column_stack(seed_preds), axis=1)
        s = score(brier_score(y, consensus), y.mean())
        results[name] = s
        preds[name] = consensus
        print(f"  {name} corrected: {s:.2f} (gain {s - base_score:+.2f})")

    # ExtraTrees + RandomForest 블렌드 (둘 다 배깅 계열이라 상관 낮을 수도 있음)
    et_rf_blend = np.clip((preds["ExtraTrees"] + preds["RandomForest"]) / 2, 0, 1)
    s = score(brier_score(y, et_rf_blend), y.mean())
    print(f"  ET+RF blend: {s:.2f} (gain {s - base_score:+.2f})")
    print(f"  corr(ET, RF): {np.corrcoef(preds['ExtraTrees'], preds['RandomForest'])[0,1]:.4f}")
    print(f"  corr(ET, LGB): {np.corrcoef(preds['ExtraTrees'], preds['LightGBM'])[0,1]:.4f}")
...
def main():
    ...
    print("2023->2024:", r1)
    print("2022->2023:", r2)
    print("(비교: ExtraTrees -- 2023->2024 801.93, 2022->2023 755.63)")
